...
describe("LogLevelUtil", ()=> {
  ...
  describe("#TRACE", ()=> {
    it("TRACE should return 'TRACE'", function() {
      expect(LogLevelUtil.TRACE).to.equal(TRACE);
    });
  });

  describe("#DEBUG", ()=> {
    it("DEBUG should return 'DEBUG'", function() {
      expect(LogLevelUtil.DEBUG).to.equal(DEBUG);
    });
  });
  
  describe("#INFO", ()=> {
    it("INFO should return 'INFO'", function() {
      expect(LogLevelUtil.INFO).to.equal(INFO);
    });
  });
  
  describe("#WARN", ()=> {
    it("WARN should return 'WARN'", function() {
      expect(LogLevelUtil.WARN).to.equal(WARN);
    });
  });
  
  describe("#ERROR", ()=> {
    it("ERROR should return 'ERROR'", function() {
      expect(LogLevelUtil.ERROR).to.equal(ERROR);
    });
  });
...
// Utilities:
// We store constants that should be defined by the LogLevelUtil class:
