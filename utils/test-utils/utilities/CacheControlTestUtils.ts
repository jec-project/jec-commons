...
/*!
* This module constains utilities used by the CacheControlTest test suite.
*/

// Utilities:
const LOADER:ClassLoader = new DefaultClassLoader();
const VALID_CLASS:string = process.cwd() + "/utils/test-utils/classes/CacheControlTestClass";
export const URL_PATTERN:string = "/url/pattern/*";
export const CACHE_CONTROL_PARAMS:CacheControlPolicy = {
  urlPattern: URL_PATTERN
};
class CacheControlDecorator implements Decorator {
  decorate(target:any, params:CacheControlPolicy):any { return target; }
}
export const CACHE_CONTROL_DECORATOR:Decorator = new CacheControlDecorator();
...
export const initContext:Function = function():JcadContext {
  ...
  let connector = new ConfigConnector(ConfigConnectorRefs.CACHE_CONTROL_CONNECTOR_REF, CACHE_CONTROL_DECORATOR);
  let context:JcadContext = factory.create();
  DecoratorConnectorManager.getInstance().addConnector(connector, context);
  JcadContextManager.getInstance().addContext(ConfigConnectorRefs.CACHE_CONTROL_CONNECTOR_REF, context);
  return context;
}
export const resetContext:Function = function(context:JcadContext):void {
  JcadContextManager.getInstance().removeContext(ConfigConnectorRefs.CACHE_CONTROL_CONNECTOR_REF);
  DecoratorConnectorManager.getInstance().removeConnector(ConfigConnectorRefs.CACHE_CONTROL_CONNECTOR_REF, context);
  context = null;
}
